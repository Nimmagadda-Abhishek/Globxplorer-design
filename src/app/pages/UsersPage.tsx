...
export function UsersPage() {
  ...
  const handleToggleLock = async (user: any) => {
    ...
    try {
      await adminApi.users.lockUser(id, nextIsLocked);
      fetchUsers();
      toast.success(nextIsLocked ? "User locked successfully" : "User unlocked successfully");
    } catch (err) {
      console.error("Failed to update user lock status:", err);
      toast.error("Unable to update user lock status. Please try again.");
    }
  };
  ...
  const handleEditUser = async (user: any) => {
    ...
    const newEmail = window.prompt("Enter new email for this user:", user.email);
    if (newEmail === null) return;

    const newPassword = window.prompt("Enter new password for this user (leave blank to keep current):", "");
    const payload: Record<string, any> = {};
    if (newEmail.trim()) payload.email = newEmail.trim();
    if (newPassword) payload.password = newPassword;

    if (!payload.email && !payload.password) {
      toast.error("Please provide an email or password to update.");
      return;
    }

    try {
      await adminApi.users.updateCredentials(id, payload);
      fetchUsers();
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-[#F3F4F6]">
              {users.map((user, index) => {
                const isLocked = typeof user.isLocked === 'boolean' ? user.isLocked : undefined;
                const statusText = typeof isLocked === 'boolean' ? (isLocked ? 'Locked' : 'Active') : (user.status || 'Unknown');
                const badgeClasses = (statusText === 'Active' || statusText.toLowerCase() === 'active') ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600';
                ...
                return (
                  <tr key={user._id || user.id || index} className="hover:bg-gray-50 transition-colors group">
                    ...
                    <td className="px-6 py-5">
                      <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase ${badgeClasses}`}>
                        {statusText}
                      </span>
                      {isLocked && (
                        <div className="text-xs text-red-600 mt-1">Locked — user cannot access the system</div>
                      )}
                    </td>
                    <td className="px-6 py-5">
                    ...
                    <td className="px-6 py-5 text-right">
                      <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button onClick={() => handleEditUser(user)} title="Edit User" className="p-2 text-[#6B7280] hover:text-[#4F46E5] hover:bg-indigo-50 rounded-lg transition-colors">
                          <Edit2 className="w-4 h-4" />
                        </button>
                        <button onClick={() => handleToggleLock(user)} title={statusText === "Active" ? "Lock User" : "Unlock User"} className="p-2 text-[#6B7280] hover:text-[#111827] hover:bg-gray-100 rounded-lg transition-colors">
